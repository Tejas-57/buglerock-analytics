...
function computeAdvancedRisk(B, funds, snapshots, weights) {
  ...
  const monthlyMean = (B.return_3y || 8) / 12 / 100 * 100;
  const cvar95 = 2.063 * monthlyStd;
  const es99 = 2.665 * monthlyStd;
  const worst3m = -1.645 * monthlyStd * Math.sqrt(3);
  const worst6m = -1.645 * monthlyStd * Math.sqrt(6);
  const probLoss = monthlyMean > 0 ? 100 * (1 - normCdf(monthlyMean / monthlyStd)) : 50;
  const maxDD = -std3y * 2;
  const ulcer = std3y * 0.7;
...
export default function PortfolioXRay({ B, AC, funds, weights, snapshots = {}, benchmarks = [], bmRets, ips, overlapData, histVar, histVarLoading, stressData, bmStress }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/holdings/debt-composition?isins=${isins}&weights=${weights_}&bond_pcts=${bondPcts}`)
      ...
      .then(d => {
        setDebtComposition({
          cqBreakdown:     d.cq_breakdown,
          secBreakdown:    d.sec_breakdown,
          avgCqLabel:      d.avg_credit_quality,
          totalDebtWeight: d.debt_weight_pct,
        });
  ...
  return (
    <div>
            ...
            <thead><tr>
              {['Fund', 'Category', 'Weight', '1Y return', 'BR Rank'].map((h, i) => (
                <th key={i} style={{
                  ...
                  padding: '8px 12px', fontSize: 9, fontWeight: 700, letterSpacing: '.05em', textTransform: 'uppercase',
                  color: 'var(--brand-mid, #A795AE)', background: 'var(--bg-secondary)', borderBottom: '2px solid var(--border)',
                }}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
                  ...
                  <tr key={f.isin} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '7px 10px', textAlign: 'center' }}>
                      {ranking && ranking !== '-' && RANK_STYLE[ranking] ? (
                        <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 700, fontFamily: 'var(--font-mono)', background: RANK_STYLE[ranking].bg, color: RANK_STYLE[ranking].color, border: '1px solid ' + RANK_STYLE[ranking].border }}>{ranking}</span>
      ...
      <div className="ptf-card" style={{ marginBottom: 14, padding: '4px 16px' }}>
        {periods.map((p) => {
          const v = B[p.k];
          return <MetricRow key={p.k} label={p.l} plainDesc={null} value={v != null ? fp(v) : '—'} />;
        })}
          ...
          {cyKeys.map((k, i) => {
            ...
            return (
              <div key={k} style={{ background: 'var(--bg-secondary)', padding: '8px 10px', borderRadius: 'var(--radius-md)', textAlign: 'center' }}>
                ...
                <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: 13, color: v >= 0 ? 'var(--pos)' : 'var(--neg)' }}>{v != null ? fp(v) : '—'}</div>
                {diff != null && <div style={{ fontSize: 9, color: diff >= 0 ? 'var(--pos)' : 'var(--neg)', marginTop: 2 }}>{diff >= 0 ? '+' : ''}{diff.toFixed(1)}% vs bm</div>}
              </div>
            );
          })}
        </div>
        {cyTotal > 0 && <div style={{ fontSize: 11, color: 'var(--text-secondary)', padding: '6px 0 4px', borderTop: '1px solid var(--border)' }}>Beat benchmark in <strong>{cyBeat}</strong> of <strong>{cyTotal}</strong> calendar years.</div>}
        {benchmarks && benchmarks.length > 0 && (
          <div style={{ fontSize: 10.5, color: 'var(--text-muted)', padding: '6px 0 4px', borderTop: '1px solid var(--border)', fontStyle: 'italic' }}>
            Benchmark: {benchmarks.length === 1 ? benchmarks[0].display_name : benchmarks.map(b => `${b.display_name} (${b.weight}%)`).join(' + ')}
          ...
          {debtLoading ? (
            ...
            <div className="ptf-card">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, padding: '16px 20px' }}>
                <div>
                  <div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', color: 'var(--brand-primary)', marginBottom: 12 }}>Credit Quality</div>
                  {debtComposition.cqBreakdown.map(({ label, pct }) => {
                    const clr = pct === 0 ? 'var(--text-muted)' : label === 'AAA / Equiv' ? '#1A7A52' : label === 'AA' ? '#2E7D32' : label === 'A' ? '#D97706' : '#C0392B';
                    const barClr = label === 'AAA / Equiv' ? '#1A7A52' : label === 'AA' ? '#4CAF50' : label === 'A' ? '#D97706' : label === 'BBB' ? '#E67E22' : '#C0392B';
                    return (
                ...
                <div>
                  <div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', color: 'var(--brand-primary)', marginBottom: 12 }}>Sector Breakdown</div>
                  {debtComposition.secBreakdown.map(({ label, pct }) => {
                    const clr = pct === 0 ? 'var(--text-muted)' : label === 'Government' ? '#3E3452' : label === 'Corporate' ? '#912F63' : label === 'Cash & Equivalents' ? '#A795AE' : '#6D5479';
                    return (
                      <div key={label} style={{ marginBottom: 10 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 3 }}>
                          <span style={{ fontWeight: 500, color: 'var(--text-primary)' }}>{label}</span>
      ...
      <div className="ptf-card" style={{ marginBottom: 14 }}>
        <div style={{ overflowX: 'auto' }}>
          {stressData?.scenarios ? (
            ...
            <div style={{ padding: '32px 16px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 12.5 }}>⏳ Computing stress scenarios from NAV history…</div>
          )}
        </div>
      ...
      {ips?.name || ips?.goal ? (
        ipsIssues.length > 0 ? (
          <div style={{ borderLeft: '4px solid var(--neg)', background: 'var(--red-dim, #FEE2E2)', padding: '12px 16px', borderRadius: '0 var(--radius-md) var(--radius-md) 0', fontSize: 11.5, color: 'var(--text-primary)', marginBottom: 14 }}>
            {ipsIssues.map((m, i) => <div key={i}>⚠ {m}</div>)}
          </div>
        ) : (
          <div style={{ borderLeft: '4px solid var(--pos)', background: 'var(--green-dim, #E6F4ED)', padding: '12px 16px', borderRadius: '0 var(--radius-md) var(--radius-md) 0', fontSize: 11.5, color: 'var(--text-primary)', marginBottom: 14 }}>
  ...
  return (
    <div style={{ margin: '26px 0 12px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 9 }}>
        <span style={{ fontFamily: 'var(--font-display)', fontSize: 11, fontWeight: 700, color: '#fff', background: 'var(--brand-primary)', borderRadius: 5, padding: '2px 8px' }}>{n}</span>
        <span style={{ fontFamily: 'var(--font-display)', fontSize: 17, fontWeight: 700, color: 'var(--brand-dark)' }}>{title}</span>
      </div>
      {sub && <div style={{ fontSize: 11.5, color: 'var(--text-muted)', margin: '4px 0 10px', lineHeight: 1.6, maxWidth: 760 }}>{sub}</div>}
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, padding: '9px 0', borderBottom: '1px solid var(--border)' }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {plainDesc && <div style={{ fontSize: 10.5, color: 'var(--text-muted)', lineHeight: 1.55, marginTop: 1 }}>{plainDesc}</div>}
      </div>
      <div style={{ textAlign: 'right', flexShrink: 0, minWidth: 78 }}>
        ...
        {verdict && <div style={{ fontSize: 9.5, fontWeight: 700, color: verdictColor || 'var(--text-muted)' }}>{verdict}</div>}
      </div>
    </div>
  );
}