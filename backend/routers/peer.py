...
# ── Allowed peer group categories (image spec, no Dividend Yield, no Contra) ──
PEER_CATEGORIES = [
    {"group": "Core equity",      "label": "Large-Cap",           "db_sub": "Large-Cap"},
    {"group": "Core equity",      "label": "Large & Mid-Cap",     "db_sub": "Large & Mid-Cap"},
    {"group": "Core equity",      "label": "Flexi Cap",           "db_sub": "Flexi Cap"},
    {"group": "Core equity",      "label": "Multi-Cap",           "db_sub": "Multi-Cap"},
    {"group": "Core equity",      "label": "Focused Fund",        "db_sub": "Focused Fund"},
    {"group": "Mid & small cap",  "label": "Mid-Cap",             "db_sub": "Mid-Cap"},
    {"group": "Mid & small cap",  "label": "Small-Cap",           "db_sub": "Small-Cap"},
    {"group": "Value & contra",   "label": "Value",               "db_sub": "Value"},
    {"group": "Value & contra",   "label": "Contra",              "db_sub": "Contra"},
    {"group": "Tax saving",       "label": "ELSS (Tax Savings)",  "db_sub": "ELSS (Tax Savings)"},
]
...
CAT_DESC = {
    ...
    "Value":             "Funds following value investment strategy — undervalued stocks with margin of safety. Benchmark: Nifty 500 TRI.",
    "Contra":            "Contrarian strategy — out-of-favour sectors and stocks. Long-term turnaround plays. Benchmark: Nifty 500 TRI.",
...
def _assign_quartiles(funds, key):
    """Return dict of isin -> quartile (1=best) based on descending sort of key."""
...
def list_peer_categories():
    """Return the allowed peer group category list grouped by section."""
...
def peer_category(
    ...
    """
    Return all funds in a peer category with stats, quartiles and distribution.
    Only categories in PEER_CATEGORIES are allowed.
    """
    ...
    try:
        ...
        # Check data exists, fall back if needed
        ...
        # Match DB sub_category to our category label
        # The HTML uses f.sub which maps to our category label directly
        # In DB, category field contains "India Fund Large-Cap" etc
        # sub is stored in category field as the suffix after "India Fund "
        ...
        funds_raw = db.query(DailyFundData).filter(
            ...
            DailyFundData.category.ilike(f"%{db_sub}%"),
        ...
        # Sort by 1Y return desc by default
