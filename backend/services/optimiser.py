...
MAX_WEIGHT = 0.15       # fallback only — user sets this via Optimise tab (default 15%)
...
def run_monte_carlo(
    ...
    def biased_sample():
        """Pure uniform sampling within per-fund bounds."""
        return np.clip(np.random.uniform(lows, highs), lows, highs)
    ...
    def run_mc(eq_min, eq_max, dt_min, dt_max, c_min, c_max, i_min, i_max, sc_mx, vol_mx):
        """Run one Monte Carlo pass targeting n_sims valid portfolios."""
        ws, rs, vs, ss = [], [], [], []
        att = 0
    ...
    def run_combined_mc(eq_min, eq_max, dt_min, dt_max, c_min, c_max, i_min, i_max, sc_mx, vol_mx):
        """Run 3 passes totalling n_sims valid portfolios — better frontier coverage."""
        all_w, all_r, all_v, all_s = [], [], [], []
        for _ in range(3):
            w_, r_, v_, s_ = run_mc(eq_min, eq_max, dt_min, dt_max, c_min, c_max, i_min, i_max, sc_mx, vol_mx)
            all_w += w_; all_r += r_; all_v += v_; all_s += s_
        return all_w, all_r, all_v, all_s

    # --- Step 1: run with all constraints (3 passes, n_sims total) ---
