"""
debug_balanced3.py — verify conservative allocation hybrid now in pool.
Run: cd buglerock-analytics/backend && python debug_balanced3.py
"""
import os, sys
...
print(f"Data date: {data_date}  Total R1/R2 funds: {len(all_f)}")
...
print(f"\nHybrid pool R1/R2 ({len(hyb_pool)} funds):")
...
for i, f in enumerate(cands):
    eq_v = get_eq(f); dbt_v = get_debt(f)
    L=_sf(f.get("large_cap")); M=_sf(f.get("mid_cap")); S=_sf(f.get("small_cap"))
    print(f"  {i+1:2d}. [{f['asset_class'][:3]}] eq={eq_v:5.1f}% debt={dbt_v:5.1f}%  L={L:4.0f} M={M:4.0f} S={S:4.0f}  {f['name'][:40]}")

eq_pct   = np.array([get_eq(f)   for f in cands])
debt_pct = np.array([get_debt(f) for f in cands])

print(f"\n=== PASS 1: No cap mix, forced_max_w={forced_max_w:.2f}% ===")
...
print(f"  Status: {'FEASIBLE' if res1.status==0 else 'INFEASIBLE'}")
if res1.status == 0:
    ...
    nz_f=np.sum(w>0)
    eff_eq=np.dot(w,eq_pct)/100; eff_debt=np.dot(w,debt_pct)/100
    print(f"  {'✓' if nz_f>=MIN_FUNDS else '✗'} {nz_f} funds  eq={eff_eq:.1f}%  debt={eff_debt:.1f}%")
...
for k in MODELS:
    try:
        ...
        fc = p["fund_count"]
        eq = p["actual"]["equity_pct"]; debt = p["actual"]["debt_pct"]
        ...
        print(f"  {flag} {p['label']:<22} {fc:2d} funds  E={e} H={h} D={d}  eq={eq:.1f}%  debt={debt:.1f}%")
