...
from backend.models.auth_models import Base, User, UserRole, SetupToken
from backend.services.auth_service import create_setup_token, send_setup_email
...
for name, email, role, is_service in USERS:
    ...
    user = User(
        ...
        password_set       = is_service,   # service account has no password
    )
    db.add(user)
